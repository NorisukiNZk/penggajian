...
<body>
    ...
    <div class="report-title">LAPORAN PEGAWAI</div> <!-- Teks laporan dengan garis bawah -->
    ...
    if (!empty($bulan) && !empty($tahun)) {
        $bulantahun = $bulan . $tahun; // Menggabungkan bulan dan tahun
    } else {
        // Jika tidak ada input, gunakan bulan dan tahun saat ini
        $bulan = date('m');
        $tahun = date('Y');
        $bulantahun = $bulan . $tahun; // Menggabungkan bulan dan tahun saat ini
    }
    ?>
    ...
    <tbody>
        <?php $no = 1; foreach ($rekapitulasi as $data): ?>
        <tr>
            <td><?php echo $no++; ?></td>
            <td><?php echo date('F', mktime(0, 0, 0, $data->bulan, 1)); ?></td> <!-- Mengubah angka bulan menjadi nama bulan -->
